...
public class Project : IProject
{
    ...
    internal Project(
        ...
        ProjectType project_type = ProjectType.ModelDesigner,
    ...
    public IProject GetSubProjectByName(string name, ProjectType type)
    {
        var command = $"get_subproject_by_name (id = \"{project_id}\", name = \"{name}\", type = \"{type.tNavValue()}\")\n";
        process.process_message(command);
        if (!int.TryParse((process.StandardOutput.ReadLine()?.OneToOneToUTF8()??"").Trim(), out int new_id))
        {
    ...
    public List<string> GetListOfSubProjects(ProjectType type = ProjectType.NenworkDesigner)
    {
        ...
        {
            subprojects.Add((process.StandardOutput.ReadLine()?.OneToOneToUTF8() ?? "").Trim());
        }
        return subprojects;
    }
